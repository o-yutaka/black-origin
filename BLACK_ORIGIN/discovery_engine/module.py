from __future__ import annotations
from typing import List
from BLACK_ORIGIN.common import Signal, ModuleBase


class DiscoveryEngineModule(ModuleBase):
    def __init__(self):
        super().__init__("discovery_engine")

    def process(self, signals: List[Signal]) -> List[Signal]:
        enriched: List[Signal] = []
        for signal in signals:
            payload = dict(signal.payload)
            payload["discovery_engine"] = {"status": "processed", "components": ['pattern_detector', 'trend_detector', 'anomaly_detector', 'emergence_detector']}
            enriched.append(Signal(source=self.name, stage=signal.stage, payload=payload, score=signal.score + 0.01))
        return enriched
